...
const animationLogic = {
      ...
      dotRings.forEach((ring, rIdx) => {
        for (let i = 0; i < ring.count; i++) {
          const angle = (i / ring.count) * Math.PI * 2,
            p = Math.sin(time * 2 - rIdx * 0.4) * 3;
          const x = centerX + Math.cos(angle) * (ring.radius + p),
            y = centerY + Math.sin(angle) * (ring.radius + p);
          const o =
            0.4 + ((Math.sin(time * 2 - rIdx * 0.4 + i * 0.2) + 1) / 2) * 0.6;
          ctx.beginPath();
          ctx.arc(x, y, 2, 0, Math.PI * 2);
          ctx.fillStyle = MONOCHROME_FILL(o);
          ctx.fill();
        }
      });
    };
  ...
  pulseWaveShockwave: (ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
    ...
    return (timestamp) => {
      ...
      dotRings.forEach((ring) => {
        for (let i = 0; i < ring.count; i++) {
          const bAngle = (i / ring.count) * Math.PI * 2,
            dist = ring.radius - waveFront;
          let pF = 0;
          if (Math.abs(dist) < waveThickness / 2)
            pF = Math.max(
              0,
              Math.cos((dist / (waveThickness / 2)) * (Math.PI / 2))
            );
          let angle = bAngle;
          if (pF > 0.01)
            angle += pF * Math.sin(time * rotSpeed + i * 0.5) * rotMag;
          const s = 1.5 + pF * 1.8,
            o = 0.2 + pF * 0.7;
  ...
  pulseWaveSpiral: (ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
    ...
    return (timestamp) => {
      ...
      dotRings.forEach((ring, rIdx) => {
        ...
        for (let i = 0; i < ring.count; i++) {
          const bAngle = (i / ring.count) * Math.PI * 2,
            spiralOffset = (ring.radius / 15) * 0.4;
          const angle = bAngle + time * rotSpeed + spiralOffset;
          const pPhase =
              time * 1.5 - ring.radius / 20 + (i / ring.count) * Math.PI,
            pRadius = Math.sin(pPhase) * 3;
          const cRadius = ring.radius + pRadius;
          const x = centerX + Math.cos(angle) * cRadius,
            y = centerY + Math.sin(angle) * cRadius;
  ...
  pulseWaveBreathingGrid: (ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
    ...
    const gridOffsetX = centerX - ((gridSize - 1) * spacing) / 2,
      gridOffsetY = centerY - ((gridSize - 1) * spacing) / 2;
    ...
    const waveSpeed = 60,
      waveThickness = 40,
      maxDist = Math.hypot(centerX, centerY) + waveThickness;
    return (timestamp) => {
      if (!lastTime) lastTime = timestamp;
      const dT = timestamp - lastTime;
      lastTime = timestamp;
      ...
      dots.forEach((dot) => {
        const distCenter = Math.hypot(dot.x - centerX, dot.y - centerY),
          distWave = Math.abs(distCenter - waveCenterDist);
        let pF = 0;
  ...
  flowingEnergyBands: (ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
    ...
    const numBands = 3,
      bandWidth = CANVAS_WIDTH / 2.5,
      bandSpeed = 25,
      totalPath = CANVAS_WIDTH + bandWidth;
    return (timestamp) => {
      if (!lastTime) lastTime = timestamp;
      const dT = timestamp - lastTime;
      lastTime = timestamp;
      ...
      dotRings.forEach((ring, rIdx) => {
        for (let i = 0; i < ring.count; i++) {
          ...
          for (let b = 0; b < numBands; b++) {
            ...
            let inf = 0;
            if (distToBand < bandWidth / 2)
              inf = Math.max(
                0,
                Math.cos((distToBand / (bandWidth / 2)) * (Math.PI / 2))
              );
            maxInf = Math.max(maxInf, inf);
          }
          const s = 1.5 + maxInf * 2.0,
            o = 0.2 + maxInf * 0.7;
          ctx.beginPath();
          ctx.arc(x, y, s, 0, Math.PI * 2);
          ctx.fillStyle = MONOCHROME_FILL(o);
  ...
  pulseWaveStretched: (ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
    ...
    return (timestamp) => {
      ...
      dotRings.forEach((ring, rIdx) => {
        for (let i = 0; i < ring.count; i++) {
          const angle = (i / ring.count) * Math.PI * 2,
            waveProg = time * 1.5 - rIdx * 0.5;
          const pF = Math.sin(waveProg),
            posPulse = (pF + 1) / 2;
          const stretch = pF * 2,
            rX = 2 + Math.abs(stretch),
            rY = 2;
          const x = centerX + Math.cos(angle) * ring.radius,
            y = centerY + Math.sin(angle) * ring.radius;
  ...
  interwovenRingPulses: (ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
    ...
    return (timestamp) => {
      ...
      ringData.forEach((ring) => {
        const pulse =
          (Math.sin(time * ring.pulseSpeed + ring.phaseOffset) + 1) / 2;
        const size = 1.5 + pulse * ring.maxAmplitude * 0.5,
  ...
  spiralRadiatingPulse: (ctx, CANVAS_WIDTH, CANVAS_HEIGHT) => {
    ...
    const arms = Array.from({ length: numArms }, (_, i) => ({
      ...
      dots: Array.from({ length: dotsPerArm }, (__, j) => ({
        baseRadius: minR + j * ((maxR - minR) / (dotsPerArm - 1)),
        size: 1.0,
      })),
    }));
    return (timestamp) => {
      ...
      ctx.fillStyle = MONOCHROME_FILL(0.7);
      ctx.fill();
      const gRot = time * rotSpeed,
        pulsePeak = ((time * waveSpeed) % (maxR + waveLen)) - waveLen / 2;
      arms.forEach((arm) => {
        arm.dots.forEach((dot) => {
          ...
          const x = centerX + Math.cos(finalAngle) * dot.baseRadius,
            y = centerY + Math.sin(finalAngle) * dot.baseRadius;
          let pInf = 0;
          const dist = Math.abs(dot.baseRadius - pulsePeak);
          if (dist < waveLen / 2)
            pInf = Math.max(
              0,
              Math.cos((dist / (waveLen / 2)) * (Math.PI / 2))
            );
          const size = dot.size + pInf * 1.2,
            opacity = 0.15 + pInf * 0.6;
          ctx.beginPath();
          ctx.arc(x, y, size, 0, Math.PI * 2);
          ctx.fillStyle = MONOCHROME_FILL(opacity);
          ctx.fill();
    ...
    return (timestamp) => {
      ...
      for (let i = 0; i < numLines; i++) {
        ...
        const scanStart = Math.max(0, scanPos - scanWidth / 2),
          scanEnd = Math.min(maxLen, scanPos + scanWidth / 2);
        if (scanStart < scanEnd) {
          const sX = centerX + Math.cos(angle) * scanStart,
            sY = centerY + Math.sin(angle) * scanStart;
          const eX = centerX + Math.cos(angle) * scanEnd,
            eY = centerY + Math.sin(angle) * scanEnd;
...
const AnimationId = Object.keys(animationLogic);
...
const useCanvasAnimation = (canvasRef, animationId) => {
  useEffect(() => {
...
const AnimationCard = ({ id, title }) => {
  ...
  return (
    <div className="animation-container w-full h-full">
...
export const CircleAnimationsGrid = ({ className }) => {
  return (
    <div className={`animations-grid-container ${className}`}>
      {ANIMATIONS.map((anim) => (
